...
const CopyWarning = ({ stream }: { stream: 'video' | 'audio' }) => (
  <p role="alert" className="mb-3 text-xs text-danger">
    The {stream}{t("codec is set to copy, so these filters cannot apply. Pick a")}{stream}{t("codec on\r\n    the")}{stream === 'video' ? 'Video' : 'Audio'}{t("tab.")}</p>
)
...
export default function FiltersSection({ value, conflicts, onChange }: Props) {
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h3 className="mb-3 text-xs font-semibold tracking-wide text-muted uppercase">{t("Video")}</h3>
      ...
      <div>
        <h3 className="mb-3 text-xs font-semibold tracking-wide text-muted uppercase">{t("Colour")}</h3>
      ...
      <div>
        <h3 className="mb-3 text-xs font-semibold tracking-wide text-muted uppercase">{t("Audio")}</h3>
